{
    ...
    */
    ...
    + ''
      ${
        if new_tmux_opts != ""
        then new_tmux_opts
        else defaulttmuxopts
      }

      ${
        if new_tmux_keys != ""
        then new_tmux_keys
        else defaulttmuxkeys
      }
  ...
  configPlugins = plugins: (
    ...
    in
      if plugins == [] || ! (builtins.isList plugins)
      then ""
      else ''
        # ============================================== #
  ...
  newTMUX = tmux.overrideAttrs (prev: {
    patches =
      (lib.optionals (prev ? patches) prev.patches)
      ++ [
        (substituteAll {
  ...
  userrunsocket =
    lib.optionalString secureSocket # bash
