{
  ...
  tmuxf = tmux.override (prev: {
    ...
    passthruvars =
      (
        if prev ? passthruvars
        then prev.passthruvars
        else []
      )
      ++ (
        if wrapZSH && zdotdir != null
        then ["ZDOTDIR"]
        else []
      );
  });
  ...
  tx =
    if custom_tx_script != null
    then custom_tx_script
    else
  ...
  footcfg = let
    shelllaunch = writeShellScript "termshell" (
      if autotx
      then ''${zsh}/bin/zsh -l -c "exec ${tx}/bin/tx"''
      else ''${zsh}/bin/zsh -l''
    );
  in ''
    shell=${shelllaunch}
