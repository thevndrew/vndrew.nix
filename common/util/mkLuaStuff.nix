...
with builtins; rec {
  ...
  in
    lua_interpreter.pkgs.luaLib.toLuaModule app;
  ...
  mkLuaApp = callPackage: arguments: let
    mkLuaAppWcallPackage = {
      ...
      in {
        ...
        buildPhase = let
          binarypath =
            if pathExists "${luaEnv}/bin/luajit"
            then "${luaEnv}/bin/luajit"
            else "${luaEnv}/bin/lua";
        in
          /*
          bash
  ...
  in
    callPackage mkLuaAppWcallPackage (arguments // {toLua = inputs.nixToLua.prettyNoModify;});
}
